...
// triMoveDef
// defines the normalized moves (for triangles a rotation of
// 3 vertices), one of which is at [0,0]: An array of vertex triples,
// one element for each move type (for triangles there are 2 types since
// there are up and down triangles).
// 
// triMoveRef
// lists the reference points for each move, i.e. coordinates
// of the point to which the origin of the normalized move is to be shifted. 
//
// base = [[],[],[]] 3D basis vectors
// 
// ss 
...
// {key: an index
//  vtx: [[x1,y1],[x2,y2],[x3,y3]] triangle vertices 1->2->3->1
//  pointsString1: string for attr "polygon" before rotation
//  pointsString2: string dito after rotation 
function triMoveList (triMoveDef,triMoveRef, base, ss){
    var i, j;
    var k = 0;
    var key;
    var vtx = [];
    ...
    for( i = 0; i < triMoveRef.length; i++){
        for (j = 0; triMoveRef[i].length; j++){
